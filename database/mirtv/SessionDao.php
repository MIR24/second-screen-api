...
class SessionDao {
    ...
    function load(&$conn, &$valueObject) {
          ...
          $sql = "SELECT * FROM teleprogramm_mir WHERE (teleprogramm_id = ".$valueObject->getSessionID().") "; 
    ...
    function loadAll(&$conn) {
          ...
          $sql = "SELECT * FROM teleprogramm_mir ORDER BY start DESC ";
    ...
    function countAll(&$conn) {
          ...
          $sql = "SELECT count(*) FROM teleprogramm_mir";
          $allRows = 0;
    ...
    function searchMatching(&$conn, &$valueObject) {
          ...
          $first = true;
          $sql = "SELECT * FROM teleprogramm_mir WHERE 1=1 ";

          if ($valueObject->getSessionID() != 0) {
              if ($first) { $first = false; }
              $sql = $sql."AND teleprogramm_id = ".$valueObject->getSessionID()." ";
          }

          if ($valueObject->getShowID() != 0) {
              if ($first) { $first = false; }
              $sql = $sql."AND article_broadcast_id = ".$valueObject->getShowID()." ";
          }

          if ($valueObject->getSessionDateTime() != "") {
              if ($first) { $first = false; }
              $sql = $sql."AND start = '".$valueObject->getSessionDateTime()."' ";
          }
    ...
    function singleQuery(&$conn, &$sql, &$valueObject) {
          ...
          if ($row = $conn->nextRow($result)) {
                   ...
                   $valueObject->setShowID($row[1]); 
                   $valueObject->setSessionDateTime($row[2]); 
                   $valueObject->setTitle($row[3]); 
          } else {
    ...
    function listQuery(&$conn, &$sql) {
          ...
          while ($row = $conn->nextRow($result)) {
               ...
               $temp->setShowID($row[1]); 
               $temp->setSessionDateTime($row[2]); 
               $temp->setTitle($row[3]); 
